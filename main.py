...
def Actividad_Fisica_Año_escolar():
    df = pd.read_csv(CSV_FILE)
    Actividad_Fisica = df.pivot_table(index='Gender', columns='University_Year', values='Physical_Activity',aggfunc='mean')
    sns.heatmap(Actividad_Fisica,
                annot=True, cmap='YlOrRd', linewidths=0.9, linecolor="white").set_title('Actividad Física',
                                                                                        fontdict={'fontsize': 20,
                                                                                                'fontweight': 'bold',
...
def Cafeina_Edad():
    df = pd.read_csv(CSV_FILE)
    Cafeina_Edad=df.pivot_table(index="Age",columns="Gender",values="Caffeine_Intake",aggfunc="mean")
    print(Cafeina_Edad)
...
def Duracion_Sueño():
    df = pd.read_csv(CSV_FILE)
    sns.pairplot(df[['Sleep_Duration','Physical_Activity','Gender']],hue='Gender', corner=True)
plt.show()
...
def Exposición_Pantalla():
    df = pd.read_csv(CSV_FILE)
    Actividad_Fisica = df.pivot_table(index='Gender', columns='University_Year', values='Screen_Time',aggfunc='mean')
    sns.heatmap(Actividad_Fisica,
...
def Exposición_Pantalla_edad():
    df = pd.read_csv(CSV_FILE)
    Actividad_Fisica = df.pivot_table(index='Gender', columns='Age', values='Screen_Time',aggfunc='mean')
    sns.heatmap(Actividad_Fisica,
